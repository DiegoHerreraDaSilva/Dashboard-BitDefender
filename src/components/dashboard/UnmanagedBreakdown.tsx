...
export function UnmanagedBreakdown({ state, className }: UnmanagedBreakdownProps) {
  ...
  return (
    <Card className={`p-6 flex flex-col gap-4 ${className ?? ""}`}>
      <h2 className="text-2xl font-semibold tracking-tight">Não gerenciados, por tipo</h2>
      {!summary ? (
        <p className="text-lg muted">Conectando à GravityZone...</p>
      ) : groups.length === 0 ? (
        <p className="text-lg muted">Nenhum dispositivo não gerenciado.</p>
      ) : (
        <ul className="flex flex-col gap-3">
          {groups.map((group) => {
            ...
            return (
              <li key={group.category} className="flex items-center justify-between gap-4 text-xl">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <span className="text-base muted">{meta.action}</span>
                </div>
              </li>
            );
          })}
